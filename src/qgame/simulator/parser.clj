(ns qgame.simulator.parser
  ...
  (:require [qgame.utils.math :as m :refer [eval-math-string]]
            ...
            [clojure.string :as s :refer [split
                                          replace]]
            [qgame.macros :as mac :refer-macros [unless
                                                  if-let*
                                                  error!]])
  ...
  ;;;NOTE: replaced every mac/error with println
...
(defn parse-forget-rule
  ...
  (let [rule (assoc bite :type :Rule :forget true :name-target nm-target-text)
    ...
    (if target-is-word?
      rule
      (println "Target is not a word" rule))))
...
(defn parse-name-rule
  ...
  (let [rule (assoc bite :type :Rule :replace-name nm-text :target target-text)
    ...
    (if nm-is-word?
      rule
      (println "Name is not a word" rule))))
...
(defn parser-execute-token
  ...
  [name-map
   (if-let [post-rules (apply-rules name-map (:text token))]
     ...
     (println "Math evaluation error" token))])

;(mac/error! "Math evaluation error" token)
...
(defn parser-execute-rule
  ...
  (if (:forget rule)
    ...
    (if-let [target-post-rules (apply-rules name-map (:target rule))]
      ...
      [name-map (println "Math evaluation on name target error" rule)])))
...
(defn parse-qubit
  ...
  (if-let [qubit-str (match-only qubit-pattern post-rules)]
    (let [qubit-val (if (. Double isNaN (. Integer parseInt post-rules))
                                (qubit-letter-to-index qubit-str)
                                (. Integer parseInt post-rules))]
      (assoc token :type :Qubit :value qubit-val))
    (println "Qubit parse failure" token)))

;(defn parse-qubit
;  "Converts a token to a qubit."
;  [{post-rules :post-rules :as token}]
;  (if-let [qubit-str (match-only qubit-pattern post-rules)]
;    (let [qubit-val (mac/unless .isNaN
;                                (. Integer parseInt post-rules)
;                                (qubit-letter-to-index qubit-str))]
;     (assoc token :type :Qubit :value qubit-val))
;    (println "Qubit parse failure" token)))
...
(defn parse-param ;;added doall print - gprice
  "Converts a token to a function parameter."
  [{post-rules :post-rules :as token}]
  ;(doall (println "param-pattern  = " param-pattern "\npost-rules = " post-rules "\n")
  (if-let [param-str (match-only param-pattern (str post-rules))]
    (assoc token :type :Param :value (.parseFloat param-str))
    (println "Param parse failure" token)))
  ;)
...
(defn parse-bit
  ...
  (when-let [bit-str (match-only bit-pattern post-rules)]
    ...
    #_(println "Bit parse failure" token))) ;Don't want to error here, need to figure out different way of erroring when with_oracle doesn't receive the proper arguments

(defn taste ;;added doall - gprice
  "Parses any tokens with the given parse-with, spitting up non-tokens."
  [parse-with bite]
  ;(doall (println "parse-with = " parse-with "\nbite = " bite "\n")
  (if-let [token (mac/unless #(not= :Token (:type %)) bite)]
    (parse-with token)
    (println "Unrecognized argument" bite)))
  ;)
...
(defn package-parse-time
  ...
  (if (= (get-in fnc [:fn-meta :name]) "with_oracle")
    (let [[swallowed bites-] (swallow parse-bit bites)
      ...
      [nil bites-])
    (println "Uncrecognized parse time function" fnc)))
     ...
     (let [[new-program* remaining*]
           (if-let [token (mac/unless #(not= :Token (:type %)) bite)]
             (if-let [fnc (->> token :post-rules (get @canonical-functions))]
               ...
               [(conj new-program (println "Lonely" token)) remaining])
